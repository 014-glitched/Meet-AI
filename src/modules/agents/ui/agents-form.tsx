...
const AgentsForm = ({ onSuccess, onCancel, initialValues } : AgentFormProps ) => {
  ...
  const createAgent = useMutation(trpc.agents.create.mutationOptions({
    onSuccess: async () => {
      // The cached data for agents.getMany is now outdated.
      // Fetch fresh data from the backend next time it’s needed.
      await queryClient.invalidateQueries(
        trpc.agents.getMany.queryOptions()
      )

      // In CREATE mode → getOne is never used, so no need.
      // In EDIT mode → a specific agent was updated → refresh it.
      if(initialValues?.id){
      await queryClient.invalidateQueries(
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea 
                  {...field} 
                  placeholder='You are a helpful coding assistant that can make me become a good Software Developer' 
                />
              </FormControl>
              <FormMessage />
            </FormItem>
